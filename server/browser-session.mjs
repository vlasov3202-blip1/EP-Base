...
function csrfOk(req,cookies){const sent=Buffer.from(String(req.headers['x-csrf-token']||''));const expected=Buffer.from(String(cookies[CSRF_COOKIE]||''));return Boolean(sent.length&&sent.length===expected.length&&crypto.timingSafeEqual(sent,expected))}
