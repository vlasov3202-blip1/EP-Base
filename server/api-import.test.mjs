...
const apiCtx1=await keys.authenticate(made.token);assert.equal(apiCtx1.companyId,'c1');assert.doesNotThrow(()=>keys.requireScope(apiCtx1,'products:read'));assert.throws(()=>keys.requireScope(apiCtx1,'orders:read'),/missing scope/);
await keys.authenticate(made.token);await assert.rejects(()=>keys.authenticate(made.token),e=>e.code==='RATE_LIMITED');
...
assert.equal(result.imported,1);const inv=await store.tenant(owner).get('InventoryUnit','inv:p1');assert.equal(inv.needsPlacement,true);assert.equal(inv.storageAddress,'');assert.equal(await store.tenant(other).get('Product','p1'),null);
await keys.revoke(owner,made.id);await assert.rejects(()=>keys.authenticate(made.token),e=>e.code==='INVALID_API_KEY');
