...
export class FeatureFlagService{
  ...
  async put(ctx,input={}){const rec={id:input.id||`flag_${crypto.randomUUID()}`,key:input.key,kind:input.kind||'feature',enabled:Boolean(input.enabled),tenantIds:[...new Set(input.tenantIds||[])],regions:[...new Set(input.regions||[])],categories:[...new Set(input.categories||[])],percentage:Math.max(0,Math.min(100,Number(input.percentage??100))),status:input.status||'active',createdAt:input.createdAt||this.now().toISOString(),updatedAt:this.now().toISOString()};if(!rec.key)throw new Error('flag key required');await this.repoFactory(ctx).put('FeatureFlag',rec);return rec;}
