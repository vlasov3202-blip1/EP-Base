...
let out=await call('POST','/api/auth/register',{companyId:'api-c1',userId:'attacker',email:`attack-${Date.now()}@test.local`,password:'password123',role:'admin'});assert.equal(out.status,201);assert.notEqual(out.body.companyId,'api-c1');assert.equal(out.body.role,'owner');assert.equal(out.body.user.role,'owner');
...
await rt.store.tenant({companyId:'api-c1'}).put('Product',{id:'p1',name:'Tenant A product'});await rt.store.tenant({companyId:'api-c2'}).put('Product',{id:'p1',name:'Tenant B product'});

out=await call('GET','/api/v1/products',null,null);assert.equal(out.status,401);
out=await call('POST','/api/v1/products',{id:'p2',name:'raw write'},a.token);assert.equal(out.status,405);assert.equal(out.body.code,'DOMAIN_COMMAND_REQUIRED');
...
out=await call('POST','/api/v1/products',{id:'p2',name:'forbidden'},s.token);assert.equal(out.status,405);
