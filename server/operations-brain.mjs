...
export class OperationsBrain{
  ...
  async analyze(ctx){const s=await this.snapshot(ctx);const signals=[];
    ...
    const stuck=s.shipments.filter(x=>['created','in_transit'].includes(x.status)&&x.updatedAt&&Date.now()-Date.parse(x.updatedAt)>48*3600_000);if(stuck.length)signals.push(sig('logistics','bad',`${stuck.length} отправлений без обновления более 48 часов`,{shipmentIds:stuck.map(x=>x.id)},'sync_logistics'));
