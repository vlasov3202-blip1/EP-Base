...
export class VisionAnonymizer {
  async anonymize(payload){
    // Provider-neutral hook. Current baseline deliberately strips metadata and identifiers.
    // Pixel-level face/license-plate redaction can be plugged in here before frames leave the trusted boundary.
    return minimizeVisionPayload(payload);
  }
}
...
export class OpenAIResponsesGateway {
  ...
  async understand({frames,voiceText,locale='ru-RU'}){
    ...
    const body={
      model:this.model,
      store:false,
      instructions:'You are EINEIRO Vision Search. Infer product-search intent from camera frames plus speech. Do not identify people. Ignore faces, names, addresses, license plates and other personal identifiers. Focus only on objects, product-relevant attributes and the user problem. If confidence is low, request one concise clarification.',
      input:[{role:'user',content}],
      text:{format:{type:'json_schema',name:'eineiro_vision_intent',strict:true,schema:DEFAULT_SCHEMA}}
    };
    ...
    return {...parsed,provider:'openai',model:this.model,responseId:json.id,usage:json.usage||null};
  }
}

export class VisionSearchService {
  constructor({anonymizer=new VisionAnonymizer(),provider,search,events=null,audit=null}={}){
    if(!provider)throw new Error('vision provider required');
    if(typeof search!=='function')throw new Error('search function required');
    this.anonymizer=anonymizer;this.provider=provider;this.search=search;this.events=events;this.audit=audit;
  }
  async resolve(ctx,input){
    ...
    const offers=mode==='search'?await this.search(ctx,{query:intent.searchQuery,category:intent.category,attributes:intent.attributes,limit:3}):[];
    this.events?.emit?.(ctx,'vision.intent',{requestId:safe.requestId,mode,confidence:intent.confidence,category:intent.category});
    this.audit?.write?.(ctx,{action:'vision.resolve',entity:'VisionRequest',entityId:safe.requestId,meta:{mode,confidence:intent.confidence,offers:offers.length}});
    return {requestId:safe.requestId,mode,intent,offers:offers.slice(0,3)};
  }
}
