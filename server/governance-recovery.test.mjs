...
repo.put(ctx,'Product',{id:'p1',name:'Товар',description:'Описание',attributes:{brand:'A'},documents:[]});const moderation=new ModerationService({repoFactory:()=>wrap});const mod=await moderation.review(ctx,{productId:'p1',categorySchema:{mandatoryFields:['brand'],documents:['certificate']}});assert.equal(mod.decision,'SELLER_ACTION_REQUIRED');assert.equal(mod.requiredActions.some(x=>x.documentType==='certificate'),true);
