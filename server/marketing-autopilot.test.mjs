...
const svc=new MarketingAutopilotService({repoFactory:()=>wrap,now:()=>new Date('2026-09-13T12:00:00Z')});
const campaign=await svc.createCampaign(ctx,{name:'Осень',channels:['eineiro_market','avito'],products:['P1'],dailyBudget:3000});
const creatives=await svc.generateCreatives(ctx,{campaignId:campaign.id,count:3});assert.equal(creatives.length,3);
