...
const engine=new UniversalImportEngineV2({repoFactory:()=>wrap,categorySchema:{validateProduct:async(_c,p)=>({valid:p.name!=='Запрещено',missing:[],errors:p.name==='Запрещено'?['bad']:[]})},moderation:{checkProduct:async()=>({allowed:true,issues:[]})}});
const result=await engine.importRows(ctx,{rows,mapping:suggested.mapping,reverseInventory:true,defaultSellerId:'s1'});assert.equal(result.imported,1);assert.equal(result.errors.length,1);const inv=(await wrap.list('InventoryUnit'))[0];assert.equal(inv.placementStatus,'unassigned');
const product=(await wrap.list('Product'))[0],offer=(await wrap.list('Offer'))[0],sku=(await wrap.list('SKU'))[0];assert.equal(product.condition,undefined);assert.equal(offer.condition,'USED');assert.equal(sku.condition,'USED');
const duplicate=await engine.importRows(ctx,{rows:[rows[0]],mapping:suggested.mapping,reverseInventory:true,defaultSellerId:'s1'});assert.equal(duplicate.imported,0);assert.equal(duplicate.duplicates,1);
