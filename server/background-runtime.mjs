...
export async function runBackgroundCycle(){
  ...
  let moderationChecked=0,moderationRouted=0;for(const companyId of companyIds){const ctx={companyId,userId:'system:scheduler',role:'owner'};const scheduler=new ScheduleService({repoFactory:c=>rt.store.tenant(c),queue});await scheduler.ensureDefaultBackupSchedule(ctx);scheduled+=(await scheduler.tick(ctx)).length;worked+=(await queue.work(ctx,{limit:20})).length;const moderation=await rt.moderationMonitoring.scan(ctx);moderationChecked+=moderation.checked;moderationRouted+=moderation.routed;}
  return{companies:companyIds.length,scheduled,worked,moderationChecked,moderationRouted};
}
