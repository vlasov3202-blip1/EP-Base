...
const p=normalizeAutoPart({name:'Фара',category:'Оптика',oeNumbers:['1','1','2'],crossNumbers:['x'],photos:['a','b']});assert.deepEqual(p.oeNumbers,['1','2']);
...
const accepted=await svc.acceptByPhoto(ctx,{photos:['1','2','3','4','5'],donorId:'d1'});assert.equal(accepted.mode,'new');assert.equal(accepted.product.photos.length,4);assert.equal(accepted.product.applicability[0].model,'Focus III');
