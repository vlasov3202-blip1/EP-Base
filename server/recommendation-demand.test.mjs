...
const os=new OfferService({repoFactory:()=>wrap});const offer1=await os.create(ctx,{id:'o1',productId:'p1',sellerId:'s1',price:30000,stock:4,deliveryOptions:[{nationwide:true}],visualAssetReady:true,visualQualityScore:90});const offer2=await os.create(ctx,{id:'o2',productId:'p2',sellerId:'s1',price:20000,stock:4,deliveryOptions:[{nationwide:true}],visualAssetReady:true,visualQualityScore:90});repo.put(ctx,'Offer',{...offer1,moderationStatus:'AUTO_APPROVED',moderationCaseId:'mod-o1'});repo.put(ctx,'Offer',{...offer2,moderationStatus:'AUTO_APPROVED',moderationCaseId:'mod-o2'});
