...
import {ConnectorRegistry,ConnectorRuntime} from './connector-framework.mjs';
...
async function connectorRuntime({ctx,channel,store,config}){
  const adapter=await createLiveChannelAdapter({ctx,channel,store,configService:config});const registry=new ConnectorRegistry();registry.register(bridgeChannelAdapter(adapter,{connectorId:channel,type:'messaging'}));return new ConnectorRuntime({registry,repoFactory:c=>store.tenant(c)});
}
...
export async function handleInboxApi(req,res){
  ...
  try{
    ...
    const sync=url.pathname.match(/^\/api\/v1\/inbox\/sync\/([^/]+)$/);if(req.method==='POST'&&sync){
      const channel=decodeURIComponent(sync[1]);const runtime=await connectorRuntime({ctx,channel,store,config});const syncId=`inbox-sync:${channel}`;const state=await repo.get('ChannelSyncState',syncId)||{id:syncId,channel,cursor:null};const minuteBucket=Math.floor(Date.now()/60000);
      const execution=await runtime.execute(ctx,{connectorId:channel,capability:'receive_messages',input:{cursor:state.cursor},idempotencyKey:`${syncId}:${state.cursor||'start'}:${minuteBucket}`,maxAttempts:3});const batch=execution.result||{};
      let imported=0,duplicates=0;const existing=await repo.list('InboxMessage');const keys=new Set(existing.map(x=>x.dedupKey));for(const raw of batch.items||[]){const externalMessageId=raw.externalMessageId||raw.id;const conversationExternalId=raw.externalConversationId||raw.conversationId;if(!externalMessageId||!conversationExternalId)continue;const dedupKey=hash(`${channel}:${externalMessageId}`);if(keys.has(dedupKey)){duplicates++;continue}const rec={id:crypto.randomUUID(),channel,direction:'inbound',externalMessageId:String(externalMessageId),conversationExternalId:String(conversationExternalId),customerExternalId:String(raw.senderId||'unknown'),text:String(raw.text||''),attachments:raw.attachments||[],receivedAt:raw.occurredAt||new Date().toISOString(),dedupKey,status:'received'};await repo.put('InboxMessage',rec);keys.add(dedupKey);imported++}
      await repo.put('ChannelSyncState',{...state,cursor:batch.cursor??null,lastExecutionId:execution.id,lastSyncedAt:new Date().toISOString()});return json(res,200,{channel,imported,duplicates,cursor:batch.cursor??null,connectorExecutionId:execution.id});
    }
    if(req.method==='POST'&&url.pathname==='/api/v1/inbox/send'){
      const p=await body(req);if(!p.channel||!p.conversationExternalId||!p.text||!p.clientRequestId)throw Object.assign(new Error('channel, conversationExternalId, text and clientRequestId required'),{status:400});const idempotencyKey=hash(`${ctx.companyId}:${p.channel}:${p.clientRequestId}`);const existing=(await repo.list('OutboundMessage')).find(x=>x.idempotencyKey===idempotencyKey);if(existing)return json(res,200,{duplicate:true,message:existing});
      const runtime=await connectorRuntime({ctx,channel:p.channel,store,config});const execution=await runtime.execute(ctx,{connectorId:p.channel,capability:'send_message',input:{message:{externalConversationId:p.conversationExternalId,text:p.text,attachments:p.attachments||[]}},idempotencyKey,maxAttempts:4});const remote=execution.result||{};
      const rec={id:crypto.randomUUID(),channel:p.channel,conversationExternalId:String(p.conversationExternalId),text:String(p.text),attachments:p.attachments||[],clientRequestId:String(p.clientRequestId),idempotencyKey,status:'sent',attempts:execution.attempt||1,sentAt:new Date().toISOString(),externalMessageId:remote.externalMessageId||null,connectorExecutionId:execution.id};await repo.put('OutboundMessage',rec);return json(res,201,{duplicate:false,message:rec});
    }
