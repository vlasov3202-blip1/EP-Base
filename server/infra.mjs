...
export class JobQueue{
  ...
  list(ctx,{status=null}={}){return this.jobs.filter(j=>j.companyId===ctx.companyId&&(!status||j.status===status)).map(structuredClone);}
...
export class BackupService{
  ...
  async create({scope='platform',companyId=null}={}){await mkdir(this.backupDir,{recursive:true});const stamp=this.now().toISOString().replace(/[:.]/g,'-');const suffix=scope==='company'&&companyId?`company-${companyId}`:'platform';const file=path.join(this.backupDir,`${stamp}-${suffix}.json`);await copyFile(this.dataFile,file);const meta={file,path:file,scope,companyId,createdAt:this.now().toISOString()};await writeFile(`${file}.meta.json`,JSON.stringify(meta,null,2));await this.prune();return meta;}
  ...
  async restore(file){const full=path.isAbsolute(file)?file:path.join(this.backupDir,file);await copyFile(full,this.dataFile);return{restoredFrom:full,restoredAt:this.now().toISOString()};}
