...
export class OfferRankingService{
  constructor({repoFactory,now=()=>new Date(),visualThreshold=60}={}){if(typeof repoFactory!=='function')throw new Error('repoFactory required');this.repoFactory=repoFactory;this.now=now;this.visualThreshold=visualThreshold;}
  async rank(ctx,{offerIds=null,intent={},userContext={},region=null,limit=100}={}){
    ...
    for(const offer of offers){const seller=sellerScores.get(offer.sellerId)||{score:0};const deliveryPossible=Array.isArray(offer.deliveryOptions)?offer.deliveryOptions.length>0:true;const allowedRegion=!region||!offer.region||offer.region===region||offer.deliveryOptions?.some?.(x=>x.region===region||x.nationwide);const hard=offerHardEligibility({offer,sellerScore:seller.score,visualThreshold:this.visualThreshold,deliveryPossible,allowedRegion});if(!hard.eligible)continue;
      const product=await repo.get('Product',offer.productId);const metrics=await repo.get('ProductMetrics',offer.productId)||{};const intentRel=relevance(intent,product,offer);const userRel=userRelevance(userContext,product,offer);const visual=norm(offer.visualQualityScore);const price=priceRelevance(intent,offer);const delivery=deliveryRelevance(region,offer);const sellerQ=norm(seller.score);const freshness=freshnessScore(offer.freshnessAt,this.now());const behaviour=behaviourPrediction(metrics);const repetitionPenalty=Math.max(0,Math.min(30,Number(userContext?.recentOfferIds?.filter?.(id=>id===offer.id)?.length||0)*10));const organicScore=Math.round(intentRel*.28+userRel*.12+visual*.12+price*.10+delivery*.10+sellerQ*.12+freshness*.08+behaviour*.08-repetitionPenalty);
      rows.push({offerId:offer.id,productId:offer.productId,sellerId:offer.sellerId,organicScore:Math.max(0,organicScore),factors:{intentRelevance:intentRel,userRelevance:userRel,visualQuality:visual,priceRelevance:price,deliveryRelevance:delivery,sellerQuality:sellerQ,freshness,behaviourPrediction:behaviour,repetitionPenalty},paidPromotionWeight:0});
    }
...
function deliveryRelevance(region,o){if(!region)return 80;if(o.region===region)return 100;if(o.deliveryOptions?.some?.(x=>x.region===region||x.nationwide))return 90;return 50}
