...
export class OfferRankingService{
  ...
  async rank(ctx,{offerIds=null,intent={},userContext={},region=null,limit=100}={}){
    ...
    for(const offer of offers){const seller=sellerScores.get(offer.sellerId)||{score:this.newSellerBaseline,provisional:true};const deliveryPossible=Array.isArray(offer.deliveryOptions)?offer.deliveryOptions.length>0:true;const allowedRegion=!region||!offer.region||offer.region===region||offer.deliveryOptions?.some?.(x=>x.region===region||x.nationwide);const hard=offerHardEligibility({offer,sellerScore:seller.score,visualThreshold:this.visualThreshold,deliveryPossible,allowedRegion});if(!hard.eligible)continue;
